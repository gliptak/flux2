...
func (b *Bootstrap) Reconcile(ctx context.Context, installOpts install.Options, deployOpts sourcesecret.Options,
	syncOpts sync.Options, repo gitprovider.RepositoryRef, accessInfo ...gitprovider.TeamAccessInfo) error {

	var r gitprovider.UserRepository
	switch repo.(type) {
	case gitprovider.OrgRepositoryRef:
		var err error
		if r, err = b.OrgRepository(ctx, repo.(gitprovider.OrgRepositoryRef), gitprovider.RepositoryInfo{}, accessInfo...); err != nil {
			return err
		}
	case gitprovider.UserRepositoryRef:
		var err error
		if r, err = b.UserRepository(ctx, repo.(gitprovider.UserRepositoryRef), gitprovider.RepositoryInfo{}); err != nil {
			return err
		}
	}

	if err := b.ComponentConfig(ctx, repo.GetCloneURL(gitprovider.TransportTypeHTTPS), syncOpts.Branch, installOpts); err != nil {
		return err
	}

	if err := b.DeployKey(ctx, r, deployOpts); err != nil {
		return err
	}
...
func (b *Bootstrap) DeployKey(ctx context.Context, repo gitprovider.UserRepository,
	...
	b.log.Actionf("determining if deploy secret %s exists", secretKey)
	...
	if ok && len(options.CAFilePath+options.PrivateKeyPath+options.Username+options.Password) == 0 {
		b.log.Successf("deploy secret up to date")
		return nil
	}

	// Generate deploy secret
	b.log.Actionf("generating deploy secret")
	...
	if err := yaml.Unmarshal([]byte(manifest.Content), &secret); err != nil {
		return fmt.Errorf("failed to unmarshal generated deploy secret manifest: %w", err)
	}

	// Upsert private key
	privateKey, ok := secret.StringData[sourcesecret.PrivateKeySecretKey]
	if ok {
		if repo != nil {
			b.log.Actionf("reconciling deploy key configuration for %s", repo.Repository().GetIdentity())
		...
		b.log.Successf("public key: %s", strings.TrimSpace(secret.StringData[sourcesecret.PublicKeySecretKey]))
	}

	// Apply deploy secret
	b.log.Actionf("applying deploy secret %s", secret.Name)
	if err = b.reconcileSecret(ctx, secret); err != nil {
		return err
	}

	b.log.Successf("reconciled deploy key")
	return nil
}

func (b *Bootstrap) ComponentConfig(ctx context.Context, url, branch string, options install.Options) error {
	...
	manifests, err := install.Generate(options, "")
	if err != nil {
...
func (b *Bootstrap) SyncConfig(ctx context.Context, url, branch string, options sync.Options) error {
	...
	if err := wait.PollImmediate(b.pollInterval, b.timeout,
		b.kustomizationReconciled(ctx, client.ObjectKey{Name: options.Name, Namespace: options.Namespace}, &kustomization, options.Branch, commit)); err != nil {
